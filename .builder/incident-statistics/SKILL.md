...
Step 0.3: Analyze the user's ORIGINAL prompt for implicit intent:
          ...
          → ASK the user:
            Question: "I found results from a previous analysis,
                       completed <TIME_AGO> ago (at <HH:MM> UTC).
                       Would you like to use this data or rerun
                       the queries from scratch?"
            Options:
              1. "Use existing data" — Resume from the previous results
              2. "Rerun from scratch" — Ignore the cache and rerun all queries

          → If user selects "Use existing data" → LOAD cache, skip to Step 3
          → If user selects "Rerun from scratch" → proceed to Step 1
```
